function [W,Gss,Gds,Hds]=ft_sphericalSplineInterpolate(src,dest,lambda,order,type,tol)
%interpolate matrix for spherical interpolation
%
% W = ft_sphericalSplineInterpolate(src,dest,lambda,order,type,tol)
%
% Inputs:
...
W=ft_sphericalSplineInterpolate(src,dest);

W=ft_sphericalSplineInterpolate(src(:,1:70),src);
clf;imagesc(W);
...
W=ft_sphericalSplineInterpolate(chPos(:,incIdx),chPos,lambda,order); % estimate the removed channels
clf;imagesc(W);
