function [fhdr,chdr,ename,czeros,cgains,cnames,fcom,ftext,coff,endian]=rd_PCAegis_hdr_v(fid)
%[fhdr,chdr,ename,czeros,cgains,cnames,fcom,ftext, coff,endian]=rd_PCAegis_hdr_v(fid)
%
...
%
%   Added endian support by Joseph Dien, 2/10/08
%
%   Added endian output, Joseph Dien 8/20/08
%
%   Changed Ldata and LCellHdr to uint, Joseph Dien 11/3/08
...
fhdr(2)=fread(fid,1,'int16',endian); %HdrVer
fhdr(3)=fread(fid,1,'uint16',endian); %LHeader
fhdr(4)=fread(fid,1,'uint32',endian); %LData
status=fseek(fid,80,'cof'); %Skip ExptNam
fhdr(5:10)=fread(fid,6,'int16',endian); %RunDate and RunTime

fhdr(11:23)=fread(fid,13,'int16',endian); %Everything else up to LCellHdr

fhdr(24:(24+fhdr(18)-1))=fread(fid,fhdr(18),'uint16',endian); %LCellHdr for each cell
...
if nargout >=2 
	...
	for loop=1:fhdr(18)
		chdr(loop,1)=fread(fid,1,'int16',endian);
		status=fseek(fid,80,'cof');
		chdr(loop, 2:5)=(fread(fid,4,'int16',endian))';
		lspectot=(chdr(loop,2)*(chdr(loop,5)/2));
		lastcol=(6+lspectot-1);
		if lastcol >= 6
		  chdr(loop,lastcol)=0;
		  chdr(loop,6:lastcol)=(fread(fid,lspectot,'int16',endian))';
		end
	end
else
	return
...
if nargout >= 3 
	...
	tempstr=fread(fid,80,'char',endian);
		warning off;
...
if nargout >= 4
	...
	czeros=fread(fid,(fhdr(19)),'int16',endian);
else
	return
end
...
if nargout >= 5
	...
	cgains=fread(fid,(fhdr(19)), 'int16',endian);
else
	return
end
...
if nargout >= 6
	...
	for loop=1:fhdr(18)
		warning off;
		cnames(loop,:)=setstr(fread(fid,80,'char',endian)');
		warning on;
...
if nargout >= 7
	...
	fcom=fread(fid,fhdr(20),'char',endian);
else
	return
end
...
if nargout >= 8
	...
	ftext=fread(fid,fhdr(21),'char',endian);
else
	return
end
